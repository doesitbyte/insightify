...
const tweet_insight = async (query, text, sentiment, user_following, replies, views, favs) => {
...
const tweet_handler = async (tweet, query) => {
    try {
        ...
        const result = await response.json();
        const sentiment = sentiment_dict[result[0][0]['label']]

        const insight = await tweet_insight(query, text, sentiment, user_following, replies, views, favs)
...
const detailed_insights = async (query, insights, avg_neutrals, avg_positives, avg_negatives, avg_favs,
...
const short_brief = async (query) => {
    ...
    try {
        ...
        for (const tweet of data) {
            const [sentiment, insight] = await tweet_handler(tweet, query)
        ...
        const det_insights = await detailed_insights(query, insights, avg_neutrals, avg_positives, avg_negatives, avg_favs
            , avg_replies, avg_user_following, avg_views)

        const cleaned_insights = det_insights.replace(/\*\*/g, '');
